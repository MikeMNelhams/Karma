...
class BotController(IController):
    def __init__(self, bot: IBot):
        self.__bot = bot
        self.__response_from_prompt = {"give_away": bot.card_giveaway_index,
    ...
    def ask_user(self, prompts: list[str], output_checks: list[ResponseCondition]) -> list[Any]:
        assert len(prompts) == len(output_checks), ValueError("The prompts must have a condition for each!")
        ...
        for prompt, output_check, cast_to_type in zip(prompts, output_checks, output_type_casters):
            ...
            response = cast_to_type(response)
            answered_correctly = output_check(response)
            if not answered_correctly:
                raise TypeError(f"Bot response from: \'{self.bot.name}\' is invalid!"
                                f"\nPlease check your bot is coded properly!")
    ...
    def get_response(self, prompt: Prompt) -> Any:
        response_getter: Callable[[], Any] = self.__response_from_prompt[prompt.text]
